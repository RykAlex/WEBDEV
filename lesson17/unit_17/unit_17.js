...
function t5() {
    ...
    a5_res = a5.filter(elem => {
        if (elem % 2 == false && typeof elem == 'number') {
            return elem;
        }
    })
    return a5_res;
...
function t9() {
    a9.forEach((elem, index) => elem = a9[index] = elem.toLowerCase());
    return a9;
}
...
function t11() {
    let a11_res = [];
    a11.filter(function (item, index) {
        if (index % 2 === 0) {
            a11_res.push(item);
        }
    })
...
function t14() {
    let lower = a14.includes(a14_sym.toLowerCase());
    let upper = a14.includes(a14_sym.toUpperCase());
    return lower || upper;
...
function t15() {
    ...
    a15_res = a15.filter(a15 => a15['pnum'].length == 6);
    return a15_res;
